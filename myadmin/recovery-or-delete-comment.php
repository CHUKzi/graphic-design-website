...
$is_delete='0';
	...
	$sender='Admin';
	$history_type=',s comment recovered';
...
$query -> execute();
	...
	echo "<script type='text/javascript'>alert('comments is recovery successfully');</script>";
...
$cmt_posted=$_GET['name2'];
	...
	$sender='Admin';
	$history_type=',s comment Permanently';
...
$query -> execute();
	...
	echo "<script type='text/javascript'>alert('Comments is Permanently Deleted');</script>";
	echo "<script type='text/javascript'> document.location = 'blog-page.php'; </script>";
} 
?>
...
<head>
	...
	<title>MANAGE WEB POSTS - COMMENT DELETE or RECOVERY</title>
...
?>
        <h5 style="background:#ededed;padding:20px;"><i class="fa fa-user text-primary"></i>&nbsp;&nbsp;<b class="text-primary"><?php echo htmlentities($result->name);?> &nbsp;&nbsp; - Posted: <i><?php echo timeAgo($time_ago);?></i></b>&nbsp;&nbsp;<br><br>
<?php 
...
<br><br>
<a href="recovery-or-delete-comment.php?reco=<?php echo $result->id;?>&name=<?php echo htmlentities($result->name);?>" onclick="return confirm('Are you sure want to recovery this comment?');"><i class="fa fa-undo" style="color:green"></i> recovery</a> /

<a href="recovery-or-delete-comment.php?per=<?php echo $result->id;?>&name2=<?php echo htmlentities($result->name);?>" onclick="return confirm('Are you sure want to delete Permanently comment?');"><i class="fa fa-trash" style="color:red"></i> Permanently Delete</a>
</h5>
									</div>
								</div>
							</div>
