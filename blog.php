...
if (empty($search)) {
    header( "Location: 404.php" );
      ...
      $query .= "ip, search, country, city";
      $query .= ") VALUES (";
      $query .= "'{$get_visitor_ip}','{$search}','{$country}','{$city}'";
      $query .= ")";
...
<head>
<meta charset="utf-8">
<title>Search: <?php echo $search ; ?> | <?php echo htmlentities($main->title);?> Blogs</title>
...
<h2><i class="fa fa-search" aria-hidden="true"></i> Search <?php echo htmlentities($result_blogs_page_head->title);?> "<?php echo $search ; ?>"</h2>
</div>
...
<input type="text" class="form-control" name="search" value="<?php echo $search ; ?>" placeholder="Enter your keywords....." required>
...
<?php $query = "SELECT * FROM blogs WHERE (title LIKE '%{$search}%' OR tags LIKE '%{$search}%')"; 
...
<?php $cnt=$cnt+1; }} else { $no='<br/><center><h2>No Blog Post found</h2></center>';$img='<br/><img src="../assets/img/no-serach-found.png">';}?>
<?php echo $no;?><?php echo $img;?>
<div class="load-more">
<!--<a href="404.php" class="btn-1">View More</a>-->
</div>
</div>
</div>
</div>
